...
function getBit(b: bigint, n: bigint) {
...
async function getValue(n, load, hv, key) {
  ...
  if (c instanceof Array) {
    for (const [k, v] of c) {
      if (makeBuffers(k).toString() === makeBuffers(key).toString())
        return makeBuffers(v);
    }
  } else {
    const child = await load(c['/']);
    return getValue(
      { bitWidth: n.bitWidth, data: parseNode(child) },
      load,
      hv,
      key,
    );
...
export const find = async (data, load, key) => {
  const hash = bytesToBig(Buffer.from(sha256(key), 'hex'));
  return getValue(
    { bitWidth: 5, data: parseNode(data) },
    load,
    { num: hash, left: 256 },
    key,
  );
...
export const forEachExp = async (data, load, cb) => {
  await forEach({ bitWidth: 5, data: parseNode(data) }, load, cb);
};

export const buildArrayData = async function (data, load) {
  var dataArray = [];
  await forEach({ bitWidth: 5, data: parseNode(data) }, load, (k, v) => {
    dataArray.push([
      address.encode('t', new address.Address(k)),
      bytesToBig(makeBuffers(v)),
    ]);
  });
...
export const readVarIntExp = (bytes, offset) => {
